...
auto main(int argc, char **argv) -> int {
  ...
  std::cerr << "x: create schema: " << schema << std::endl;
...

  {
    ...
    bustub->txn_manager_->Commit(txn);
    delete txn;
    std::cout << "x: initialize data as below" << std::endl << ss.str();
  }
    ...
    threads.emplace_back(
        std::thread([verbose, thread_id, &bustub, enable_update, duration_ms, &total_metrics, bustub_nft_num] {
          ...
          while (!metrics.ShouldFinish()) {
            ...
            if (verbose) {
              fmt::print("T{}: begin: thread {} update nft {} to terrier {}\n", 1 + thread_id, thread_id, nft_id,
                         terrier_id);
            }
            ...
            if (verbose) {
              fmt::print("T{}: end  : thread {} update nft {} to terrier {}\n", 1 + thread_id, thread_id, nft_id,
                         terrier_id);
            }
  ...
  threads.emplace_back(std::thread([&bustub, duration_ms, &total_metrics, bustub_nft_num] {
    ...
    while (!metrics.ShouldFinish()) {
      ...
      if (txn_success) {
        ...
        // Due to how BusTub works for now,
        // it is impossible to get more than bustub_nft_num rows,
        // but it is possible to get fewer than that number.
        if (all_nfts_integer.size() != bustub_nft_num) {
          fmt::print("T5: unexpected result when verifying length. scan result: {}, total rows: {}.\n",
                     all_nfts_integer.size(), bustub_nft_num);
          if (bustub_nft_num <= 100) {
            fmt::print("T5: This is everything in your database:\n{}", ss.str());
          }
          exit(1);
        }
        for (int i = 0; i < static_cast<int>(bustub_nft_num); i++) {
          if (all_nfts_integer[i] != i) {
            fmt::print("T5: unexpected result when verifying \"{} == {}\",\n", i, all_nfts_integer[i]);
            if (bustub_nft_num <= 100) {
              fmt::print("T5: This is everything in your database:\n{}", ss.str());
            }
            exit(1);
        ...
        if (txn_success) {
          if (ss.str() != prev_result) {
            fmt::print("T5: ERROR: non repeatable read!\n");
            if (bustub_nft_num <= 100) {
              fmt::print(
                  "T5: This is everything in your database:\n--- previous query ---\n{}\n--- this query ---\n{}\n",
                  prev_result, ss.str());
            }
            exit(1);
          }
